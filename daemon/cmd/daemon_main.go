...
func init() {
	...
	flags.Var(option.NewNamedMapOptions(option.RateLimitName, &option.Config.RateLimit, nil), option.RateLimitName, "Rate limiting configuration (example: --rate-limit endpoint-create=rate-limit:10/m,rate-burst:2)")
	option.BindEnv(option.RateLimitName)
