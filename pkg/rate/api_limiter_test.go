...
func (b *ControllerSuite) TestMinParallelRequests(c *check.C) {
	...
	a := NewAPILimiter("foo", APILimiterParameters{
		EstimatedProcessingDuration: time.Millisecond,
		AutoAdjust:                  true,
		ParallelRequests:            10,
...
func (b *ControllerSuite) TestParseUserConfigKeyValue(c *check.C) {
	...
	c.Assert(parseUserConfigKeyValue("", "", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("foo", "", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("rate-limit", "10", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("rate-limit", "10/m", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("rate-burst", "foo", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("rate-burst", "10", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("max-wait-duration", "100sm", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("max-wait-duration", "100ms", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("min-wait-duration", "100sm", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("min-wait-duration", "100ms", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("estimated-processing-duration", "100sm", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("estimated-processing-duration", "100ms", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("auto-adjust", "not-true", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("auto-adjust", "true", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("auto-adjust", "false", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("max-parallel-requests", "ss", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("max-parallel-requests", "10", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("parallel-requests", "ss", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("parallel-requests", "10", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("min-parallel-requests", "ss", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("min-parallel-requests", "10", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("mean-over", "foo", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("mean-over", "10", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("log", "not-true", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("log", "true", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("log", "false", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("delayed-adjustment-factor", "0.25", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("delayed-adjustment-factor", "foo", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("max-adjustment-factor", "0.25", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("max-adjustment-factor", "foo", p), check.Not(check.IsNil))
	c.Assert(parseUserConfigKeyValue("skip-initial", "2", p), check.IsNil)
	c.Assert(parseUserConfigKeyValue("skip-initial", "foo", p), check.Not(check.IsNil))
}

func (b *ControllerSuite) TestParseUserConfig(c *check.C) {
	p := &APILimiterParameters{}
	c.Assert(parseUserConfig("auto-adjust:true,", p), check.IsNil)
	c.Assert(p.AutoAdjust, check.Equals, true)
	c.Assert(parseUserConfig("auto-adjust", p), check.Not(check.IsNil))
	c.Assert(parseUserConfig("1:2:3", p), check.Not(check.IsNil))
}
