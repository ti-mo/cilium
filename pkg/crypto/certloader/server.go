...
// EventualWatchedServerConfig returns a channel where exactly one
...
// least one file path.
func EventualWatchedServerConfig(log logrus.FieldLogger, caFiles []string, certFile, privkeyFile string) (<-chan *WatchedServerConfig, error) {
	if certFile == "" {
		return nil, ErrMissingCertFile
	}
	if privkeyFile == "" {
	...
	ew, err := EventualWatcher(log, caFiles, certFile, privkeyFile)
	if err != nil {
		return nil, err
	}
...
func (cfg *WatchedServerConfig) ServerConfig(base *tls.Config) *tls.Config {
	...
	return &tls.Config{
		GetConfigForClient: func(_ *tls.ClientHelloInfo) (*tls.Config, error) {
			...
			cfg.log.WithField("keypair-id", keypairId(keypair)).
